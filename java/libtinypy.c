...
tp_obj eval(TP, const char* text, tp_obj globals) {
    ...
    tp_set(tp, globals, tp_string_atom(tp, "__name__"), tp_string_atom(tp, "<eval>"));
    tp_set(tp, globals, tp_string_atom(tp, "__code__"), code);
...
tp_obj run_file(TP, const char* filename, tp_obj globals) {
    ...
    tp_params_v(tp, 1, tp_string_atom(tp, filename));
    tp_obj text = tp_load(tp);
    tp_obj code = tp_compile(tp, text, tp_string_atom(tp, "__main__"));
    tp_set(tp, globals, tp_string_atom(tp, "__name__"), tp_string_atom(tp, "__main__"));
    tp_set(tp, globals, tp_string_atom(tp, "__code__"), code);
    tp_exec(tp, code, globals);
    return tp->last_result;
}
