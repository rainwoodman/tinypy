...
tp_obj tp_repr_tracked(TP, tp_obj self) {
...
tp_obj tp_str_tracked(TP, tp_obj self) {
    /* we only put unmanaged tp_data objects to the list.*/
    tpd_list * visited = tpd_list_new(tp);
...
tp_obj tp_str_(TP, tp_obj self, tpd_list * visited) {
    ...
    int type = self.type;
    if(type == TP_DICT) {
        tp_obj data = tp_data(tp, 0, self.dict.val);
        ...
        tp_obj data = tp_data(tp, 0, self.list.val);
        if(tpd_list_find(tp, visited, data, tp_cmp) >= 0)
            return tp_string("[...]");
        tpd_list_append(tp, visited, data);
    }
    ...
    if (type == TP_STRING) { 
        ...
        if ((fabs(v-(long)v)) < 0.000001) {
            return tp_printf_tracked(tp,"%ld",(long)v);
        }
...
tp_obj tpy_str(TP) {
    tp_obj v = TP_OBJ();
    return tp_str_tracked(tp, v);
}

tp_obj tpy_repr(TP) {
    tp_obj v = TP_OBJ();
    return tp_repr_tracked(tp, v);
}
