...
void tp_deinit(TP) {
    while (tp->root.list.val->len) {
        tpi_list_pop(tp, tp->root.list.val, 0, "tp_deinit");
    }
...
tp_obj tp_call(TP,tp_obj self, tp_obj params) {
    ...
    if (self.type == TP_DICT) {
        if (self.dict.dtype == 1) {
            tp_obj meta; if (_tp_lookup(tp,self,tp_string("__new__"),&meta)) {
                tpi_list_insert(tp, params.list.val, 0, self);
    ...
    if (self.type == TP_FNC) {
        ...
        if ((self.fnc.ftype&2)) {
            ...
            tpi_list_insert(tp, params.list.val, 0, self.fnc.info->self);
        } else {
            tp->frames[tp->cur].regs[0] = params;
        }
        tp_run(tp,tp->cur);
...
tp_obj tp_args(TP, int argc, char *argv[]) {
    ...
    int i;
    for (i=1; i<argc; i++) {
        tpi_list_append(tp, self.list.val, tp_string(argv[i]));
    }
    return self;
}
