void tpi_list_realloc(TP, tpi_list *self, int len) {
    if (!len) { len=1; }
    self->items = (tp_obj*)tp_realloc(tp, self->items,len*sizeof(tp_obj));
    self->alloc = len;
}

void tpi_list_set(TP, tpi_list *self,int k, tp_obj v, const char *error) {
    if (k >= self->len) {
        tp_raise(,tp_string("(tpi_list_set) KeyError"));
    }
    self->items[k] = v;
    tp_grey(tp,v);
}

tpi_list *tpi_list_new(TP) {
    return (tpi_list*) tp_malloc(tp, sizeof(tpi_list));
}

void tpi_list_free(TP, tpi_list *self) {
...
tp_obj tpi_list_get(TP, tpi_list *self, int k, const char *error) {
    if (k >= self->len) {
        tp_raise(tp_None, tp_string("(tpi_list_get) KeyError"));
    }
    return self->items[k];
}
void tpi_list_insertx(TP, tpi_list *self, int n, tp_obj v) {
    if (self->len >= self->alloc) {
        tpi_list_realloc(tp, self,self->alloc*2);
    }
    if (n < self->len) { memmove(&self->items[n+1],&self->items[n],sizeof(tp_obj)*(self->len-n)); }
    self->items[n] = v;
    self->len += 1;
}
void tpi_list_appendx(TP, tpi_list *self, tp_obj v) {
    tpi_list_insertx(tp, self, self->len, v);
}
void tpi_list_insert(TP,tpi_list *self, int n, tp_obj v) {
    tpi_list_insertx(tp,self,n,v);
    tp_grey(tp,v);
}
void tpi_list_append(TP,tpi_list *self, tp_obj v) {
    tpi_list_insert(tp,self,self->len,v);
}

void tpi_list_extend(TP, tpi_list * self, tpi_list * v) {
    int i;
    for (i = 0; i < v->len; i++) {
        tpi_list_append(tp, self, v->items[i]);
    }
}

tp_obj tpi_list_pop(TP,tpi_list *self, int n, const char *error) {
    tp_obj r = tpi_list_get(tp,self,n,error);
    if (n != self->len-1) {
...
int tpi_list_find(TP,tpi_list *self, tp_obj v) {
    int n;
...
int tpi_list_cmp(TP, tpi_list * a, tpi_list * b)
{
    int n, v;
    for(n=0; n<_tp_min(a->len, b->len); n++) {
        ...
        if (aa.type == TP_LIST && bb.type == TP_LIST) {
            v = tpi_list_cmp(tp, aa.list.val, bb.list.val);
        } else {
            v = tp_cmp(tp, aa, bb);
        }
        if (v) { return v; }
...
tp_obj tp_list_add(TP, tp_obj a, tp_obj b)
{
    tp_obj r;
    r = tp_list_copy(tp, a);
    tpi_list_extend(tp, r.list.val, b.list.val);
    return r;
}
...
tp_obj tp_list_mul(TP, tp_obj a, int n)
{
    ...
    int i;
    for (i = 1; i < n; i ++) {
        tpi_list_extend(tp, r.list.val, a.list.val);
    }
    return r;
}
...
tp_obj tp_list_n(TP, int n, tp_obj *argv) {
    int i;
    tp_obj r = tp_list(tp);
    tpi_list_realloc(tp, r.list.val,n);
    for (i=0; i<n; i++) {
...
tp_obj tpy_list_index(TP) {
    tp_obj self = TP_OBJ();
    tp_obj v = TP_OBJ();
    int i = tpi_list_find(tp,self.list.val,v);
    if (i < 0) {
...
tp_obj tpy_list_pop(TP) {
    tp_obj self = TP_OBJ();
    return tpi_list_pop(tp, self.list.val, self.list.val->len-1, "pop");
}
...
tp_obj tpy_list_insert(TP) {
    ...
    int n = TP_NUM();
    tp_obj v = TP_OBJ();
    tpi_list_insert(tp, self.list.val, n, v);
    return tp_None;
}
